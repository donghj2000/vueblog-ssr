...
export default defineComponent({
    ...
    asyncData( { store, route }) {
        //console.log("Home,asyncData,(store,route)=");
        return store.dispatch('getArticleList').then(function () {})
    },
    setup() {
        ...
        const state = reactive ({
            ...
            total: store.state.articleCount,
            params: {
                status: "Published",
                page: 1,
                page_size:10,
        ...
        onMounted(() => {
            ...
            //服务端渲染首页成功后，如果还有后续数据，则从第2页开始请求数据
            if (state.total > state.articleList.length) {
                state.params.page++;
            } 
